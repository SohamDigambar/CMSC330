  ...
  let ast =
    ( [],
      Let
        ( "add1",
          false,
          Fun ("x", Binop (Add, ID "x", Value (Int 1))),
          FunctionCall (ID "add1", Value (Int 1)) ) )
  in
  let result = Int 2 in
  ...
  let ast =
    ( [],
      Let
        ( "apply",
          false,
          Fun ("x", Fun ("y", FunctionCall (ID "x", ID "y"))),
          Let
            ( "add1",
              false,
              Fun ("z", Binop (Add, ID "z", Value (Int 1))),
              FunctionCall (FunctionCall (ID "apply", ID "add1"), Value (Int 5))
            ) ) )
  in
  let result = Int 6 in
  let student = eval_expr_ast [] ast in
...
let public_expr_double_fun _ =
  let ast =
    ( [],
      FunctionCall
        ( FunctionCall
            (Fun ("x", Fun ("y", Binop (Add, ID "x", ID "y"))), Value (Int 5)),
          Value (Int 2) ) )
  in
  let result = Int 7 in
  let student = eval_expr_ast [] ast in
    ...
    ( [],
      Let
        ( "sanity",
          false,
          If
            ( Binop (Equal, Value (Int 1), Value (Int 1)),
              Value (Bool true),
              Value (Bool false) ),
          ID "sanity" ) )
  in
  ...
  let ast =
    ( [],
      Let
        ( "abc",
          false,
          Fun ("a", Binop (Add, ID "a", Value (Int 1))),
          FunctionCall (ID "abc", Value (Int 1)) ) )
  in
  let result = Int 2 in
  let student = eval_expr_ast [] ast in
...
let public_expr_minus_one _ =
  let ast =
    ([], Let ("x", false, Binop (Sub, Value (Int 1), Value (Int 1)), ID "x"))
  in
  let result = Int 0 in
  let student = eval_expr_ast [] ast in
...
let public_expr_simple_concat _ =
  let ast =
    ([], Binop (Concat, Value (String "Hello"), Value (String " World")))
  in
...
let public_expr_single_fun _ =
  let ast =
    ( [],
      FunctionCall (Fun ("x", Binop (Add, ID "x", Value (Int 1))), Value (Int 1))
    )
  in
  let result = Int 2 in
  let student = eval_expr_ast [] ast in
...
let public_expr_single_if _ =
  let ast =
    ( [],
      If
        ( Binop (Equal, Value (Int 1), Value (Int 2)),
          Value (Bool false),
          Value (Bool true) ) )
  in
  let result = Bool true in
...
let public_err_div_by_zero _ =
  let ast =
    ([], Let ("x", false, Binop (Div, Value (Int 1), Value (Int 0)), ID "x"))
  in
        ...
        (Let
           ( "fact",
             ...
             Fun
               ( "x",
                 If
                   ( Binop (Equal, ID "x", Value (Int 1)),
                     Value (Int 1),
                     Binop
                       ( Mult,
                         ID "x",
                         FunctionCall
                           (ID "fact", Binop (Sub, ID "x", Value (Int 1))) ) )
               ),
             FunctionCall (ID "fact", Value (Int 3)) )) )
  in
             ...
             Let
               ( "b",
                 ...
                 Binop
                   ( Add,
                     ID "a",
                     Binop (Add, ID "b", Binop (Add, ID "c", ID "d")) ) ) )) )
  in
  let result =
    ([ ("c", { contents = Int 3 }); ("d", { contents = Int 4 }) ], Some (Int 10))
  in
  let student =
    eval_mutop_ast
      [ ("c", { contents = Int 3 }); ("d", { contents = Int 4 }) ]
      ast
  in
      ...
      Let
        ( "x",
          ...
          Let
            ( "f",
              false,
              Fun ("x", Binop (Add, ID "x", Value (Int 22))),
